...
- **Fach:** SYTB
- **Thema:** Curl
- **Aufgabenstellung:** [cURL beim Notenmanagement](https://www.franzmatejka.at/htl/doc/ITSI_2_linux/13_curl.html)

---
## Inhaltsverzeichnis
```
1. Übung (Browser Developer Tool)
2. Übung (curl und Notenmanagement)
```
...
Zuerst muss man also Google Chrome öffnen und daraufhin `F12` klicken um die Developer Tools zu öffnen. Oben bei den Developer Tools dann statt Elements auf `Network` klicken.

Um den Cache dann zu deaktivieren muss man unten unter Network bei dem Punkt `Disable Cache` einen Hacken setzen.
Daraufhin die Seite neuladen. Hier sieht man daraufhin die gesamten Netzwerkanfragen. Also `html`, `css`, `Js`, `jpeg`, etc.

Danach sucht man in den Netzwerkanfragen nach der CSS Datei, das kann man filtern, indem man oben statt `All` auf `CSS` klickt, somit erscheinen nur die CSS-Dateien. In der Google Chrome Startseite sind das `4 CSS-Dateien`. Ich nehme die mit der größten Größe.

Danach einen rechtsklick auf die gewählte CSS-Datei machen und dann auf `Copy` und dann auf `Copy as cURL`. Daraufhin öffnet man Kali und das Terminal und fügt das kopierte ein. Daraufhin erscheint dieser command:
```
──(kali㉿kali)-[~]
└─$ curl 'https://www.gstatic.com/og/_/ss/k=og.qtm.vjyYcS7rQwo.L.W.O/m=qmd,qcwid,d_b_gm3,d_wi_gm3,d_lo_gm3/excm=qaaw,qabr,qadd,qaid,qalo,qebr,qein,qhaw,qhawgm3,qhba,qhbr,qhbrgm3,qhch,qhchgm3,qhga,qhid,qhidgm3,qhin,qhlo,qhlogm3,qhmn,qhpc,qhsf,qhsfgm3,qhtt/d=1/ed=1/ct=zgms/rs=AA2YrTtEJMqNNb04d2z46AITatL-Y66Oww' \
  ...
  -H 'x-client-data: CNr7ygE='
  ```
  Dieser Befehl simuliert den Browser-Request, da das Terminal nicht unterscheiden kann.
  Wenn man jetzt bei diesem Befehl Enter drückt bekommt man jetzt den CSS Inhalt:
...
Hier auch wieder Chrome starten, danach mit F12 die Developer Tools öffnen und auf den Network Tab klicken und den Cache deaktivieren.

Dann das `Notenmanagement` öffnen und einloggen. Die Netzwerkanfragen sollten währenddessen geöffnet sein. Danach auf eine Note klicken. Man sieht daraufhin viele Netzwerkanfragen.

Dann statt All auf `Fetch/XHR` klicken. Jetzt sieht man nur die `API-Requests`.
Den JSON-Request, den ich nehme ist `2966`. Das wieder `Copy as cURL` und in kali linux reinkopieren. Der Befehl sieht dann so aus:
```
MUlnB1t4jppFgwxht8oFLgERrM68YKOgIFaFI2CN5eUEV9mFMNtXIqxxWCNB6EtJ9w-twgweURJLMN8xocKnXiRiIRFTFxJ_nTfXHNl90gDsjERqjRwKD4Y-lGSXI2AjyyA08USjECCDbupLiSMBr1Ew8HzJciv69OiZ3QcKjALHobdFMJCf6WF1Av481XAjSmkXz65SDMVkLpEBU9b6Or4trMGUmCbuvp77Qkzwg69myf--gIhBH89Mk1D3ejMOSOg55EeKuC7So4Bt229bxIKHACEpKdcFULIV4ZkjqdkI1CvztZIv_4aLwjQzBNzjkoZaTkHoZ1CQuSitgtW6u6RR7g72mncV4CEgTf73dirL77TLGyHgO6TnMp_NpzwX5a5-d56KADJYJ1_c66pKB9Klja9pUqrRT3vKfPIDXuCxRZGFKJaRP4hqRYbeZBm62EVyjaC9HiQ' \ 
  -H 'cache-control: no-cache' \
  -b '.AspNet.Cookies=nkJhaOXU8TksyBXljNPb7cvTW0N7GhEQbhtY3VdOfnyxx7wMnu3KFa6JJYdwouF4_skJ3JNSHveJhWVedqDtHaOIztOG-f2WI9BY7y2gVY5zAYXaefX2RlmDZUAVllPHaixR-WTs1Ak_ZAeJOSPQbAQzLagg6c86eDisiIk-TVQ9k3Im9aWm9SFIEQoxOWkGznwVyFwm5CWf5HlzYXxT3M_0ie6udkF5XDtZRxn7F9IFfC-P3EpOhwfj34WuijCCMMcPn-9wvV7zgriKxGd4ettlKOpL7MtuKYW1bxFPIe6DFl2tobCpyWgzSlvBrYeLZgb999D3pHH6W0hoeInnRZaiP34ZOhGgLd3B-ugQKkTVmWx4uySTO7puJCjzUiL3po9_jG6pBj1_PZrDTJzXhCLFMTZh_WGr5MCGPH-9mNAPe1gOlQocEU0IciPsArf_fDBJC2cmp-VTjLLbmQrm02cGEnskJ5HAHFt4QKxSUA6vSlA-t9yScAJ6hrJuvz20' \
  -H 'pragma: no-cache' \
  -H 'priority: u=1, i' \
...
```
{"LF_ID":2966,"Datum":"2026-03-02T00:00:00","Lehrer_ID":1026,"Klasse":"3AHITS","Fach":"GGP-w","Typ":"Test","MaxPunkte":18.5,"Kommentar":"3. Test - EU","Notenspiegel":[5,1,5,3,0,1]}
```

Also der curl Command imitiert den Request vom Browser, was das ganze sehr anfällig für Hacking macht.